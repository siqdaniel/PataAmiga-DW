...
SELECT 
    'grafias distintas de categoria (no MySQL)' AS diagnostico,
    COUNT(DISTINCT `CategoriaProduto`) AS valor, 
    '18' AS esperado 
FROM stg_pedido
UNION ALL 
SELECT 
    'grafias distintas de nome de loja',
    COUNT(DISTINCT `Loja-Nome`), 
    CAST((SELECT COUNT(DISTINCT `Loja-Nome`) FROM stg_pedido) AS CHAR) 
FROM stg_pedido
UNION ALL 
SELECT 
    'grafias distintas de HouveDesconto',
    COUNT(DISTINCT `HouveDesconto`), 
    CAST((SELECT COUNT(DISTINCT `HouveDesconto`) FROM stg_pedido) AS CHAR) 
FROM stg_pedido
UNION ALL 
SELECT 
    'grafias distintas de CanalPedido',
    COUNT(DISTINCT `CanalPedido`), 
    CAST((SELECT COUNT(DISTINCT `CanalPedido`) FROM stg_pedido) AS CHAR) 
FROM stg_pedido
UNION ALL 
SELECT 
    'pedidos sem Cod Loja preenchido',
    SUM(CASE WHEN `Cod Loja` = '' THEN 1 ELSE 0 END), 
    '1575  (~39%)' 
FROM stg_pedido
UNION ALL 
SELECT 
    'pedidos sem nome de loja (vao para a -1)',
    SUM(CASE WHEN `Loja-Nome` = '' THEN 1 ELSE 0 END), 
    '3' 
FROM stg_pedido;
